...
The timer was used to give functionality to timer interrupts. This allowed the OS to call an interrupt every millisecond. The timer was also used to make the delayed send method work. To incorporate the timer with the delayed send, a timer counter had to be made. This counter would increment every time the timer interrupt entered its interrupt handler. The timer counter was used to keep track of the delayed messages being sent off, as it was needed to determine if the amount of time before the message can be sent has elapsed.
...
Processes can have one of five valid priority levels from 0 (Highest) to 4 (Lowest). The lowest priority level is reserved for the Null Process. All other priority levels can be used by any process.

Processes can also be in a number of states. Before a process is first run, it is in the \texttt{NEW} state. When a process is run it enters the \texttt{RUNNING} state. Once a process has finished running and is not blocked, it enters the \texttt{READY} state. If a process is either blocked on memory or is waiting to receive a message it either enters into the \texttt{MSG\_BLOCKED} or \texttt{MEM\_BLOCKED} state depending on what is it blocked on. When a process terminates it enters the \texttt{EXIT} state. However, no processes ever terminate in the operating system, so no processes will enter this state.
...
Once the next process is chosen, the context switch occurs. This operation occurs in two slightly different ways depending on if the operating system is switching to a new process or a process that has already ran. Additionally, none of the following will occur the process we are switching to is the current process. 
...
~\\The following figures show the histograms for each method tested depicting the values obtained per method call recorded in $\mu s$.
...
\texttt{blocks\_allocated}: This variable represents the number of memory blocks that have been allocated from the heap.

\texttt{alloc\_mem}: This variable is a pointer to the linked list of memory blocks that have been allocated from the heap.
